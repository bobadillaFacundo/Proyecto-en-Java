...
public class ABMCiudad extends Generic {
	...
	public Respuesta addCiudad(String codigo, String nombre, String provicnia) {
		...
		if (ciudades.containsKey(Integer.parseInt(codigo)))	{ return Respuesta.ERROR_EXISTE; }
	...
	public Respuesta removeCiudad(String codigo) {
		...
		if (!(ciudades.containsKey(Integer.parseInt(codigo))))	{return Respuesta.ERROR_NO_EXISTE;}
		
		Ciudad ciudad = ciudades.get(Integer.parseInt(codigo));
		
		if ((ciudad.getPersonas().size()>0) || (ciudad.getEmpresa().size()>0)) { return Respuesta.ERROR_CONSTRAIN; }
	...
	public Respuesta modificarCiudad(String codigo,String nombre, String provicnia) {
		...
		if (!(ciudad.getNombre().equals(nombre))) ciudad.setNombre(nombre);
		
		if (!(ciudad.getProvincia()).equals(provicnia)) ciudad.setProvincia(provicnia);
